    ...
    struct __spawn_future_state final
      ...
      void __consume(_Rcvr& __rcvr) noexcept {
        ...
        if (sentinel == (this + 1)) { // NOTE: we didn't update __registeredReceiver_
          ...
          //
          // We need a load-acquire if we happen to need to observe the producer's result; we don't
          // need a store-release, though, because the producer's output will be ignored if we
          // manage to abandon the operation before it completes.
          sentinel = __registeredReceiver_.exchange(this, __std::memory_order_acquire);
        }
      ...
      void __try_cancel() noexcept {
        ...
        //
        // Consider: maybe the above reasoning is incorrect; can we do this before the relaxed
        //           exchange, below?
        ...
        void* sentinel = nullptr;
        if (
          __registeredReceiver_.compare_exchange_strong(
            sentinel,
            this + 1,
            // We don't need to synchronize with anybody on success because success means we were
            // first so both __consume and __complete will synchronize with each other when they
            // run. We need to load-acquire on failure, though, to make sure we can safely read
            // __callback_ in the event that __consume ran on a different thread. Since it's UB for
            // the failure memory order to be stronger than the success order, both must be acquire.
            __std::memory_order_acquire,
            __std::memory_order_acquire)) {
        ...
        // We don't need to synchronize here so relaxed is fine. The two possible outcomes here are:
        //  1. we successfully abandon the operation and complete the receiver ourselves with
        //     set_stopped, or
        //  2. we fail to abandon the operation because the producer has completed since we CAS'd.
        //
        // In the first case, we've already synchronized with the consumer by performing a CAS with
        // load-acquire semantics and we don't need to synchronize with the producer because its
        // result will be ignored.
        //
        // In the second case, our attempt to abandon the operation is futile because the producer
        // has already finished and is busy completing the receiver; our effort may as well not have
        // happened and need not synchronize with anything.
        sentinel = __registeredReceiver_.exchange(this, __std::memory_order_relaxed);
      ...
      void __abandon() noexcept {
        ...
        void* sentinel = nullptr;
        if (__registeredReceiver_.compare_exchange_strong( // Compare to exchange(this, acquire)
              sentinel,
              this,
              // We need load-acquire semantics in the case that we're *second* because we need to see
              // the complete write to __result_ before we invoke __destroy, but, in the case we're
              // first, we're going to act as if the producer never existed so there's no need to
              // synchronize at all.
              __std::memory_order_acquire,
